"""
python-services/plate-service/src/health.py

aiohttp health endpoint — same shape as violation-service's so the
Prometheus scrape config is uniform across the fleet (spec §12.4).
"""
...
class HealthServer:
    ...
    async def _readyz(self, _req: web.Request) -> web.Response:
        checks = {
            ...
            "model":    bool(self._is_ready() if self._is_ready else True),
        }
